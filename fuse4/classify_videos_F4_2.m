...
%        imgs = zeros(IMG_DIM, IMG_DIM, 12, 5, 'single');
        scores1 = zeros(101, 10);
...
%         for h = 6:10
%             for k = 1:4
%                 imgs(:,:,(k-1)*3 + 1, h - 5) = all_imgs{k}(:,:,1,h);
%                 imgs(:,:,(k-1)*3 + 2, h - 5) = all_imgs{k}(:,:,2,h);
%                 imgs(:,:,(k-1)*3 + 3, h - 5) = all_imgs{k}(:,:,3,h);
%             end
%             input_data = {imgs};
%             s = caffe('forward', input_data);
%             scores2 = squeeze(s{1});
...
save([out_path filesep 'ucf101_AUG_VGG16_F4V2_iter40000_scores_0318.mat'], 'S');
