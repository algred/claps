...
fid = fopen(['data' filesep 'ucf101_split' num2str(split) '_train.txt'], 'r');
X = textscan(fid, '%s %d\n', -1);
fnames = X{1};
cidx = X{2};
fclose(fid);

fid = fopen(['data' filesep 'ucf101_K1_val.txt'], 'r');
X = textscan(fid, '%s %d\n', -1);
fnames_val = X{1};
cidx_val = X{2};
fclose(fid);
...
all_fnames = [fnames; fnames_val; aug_fnames(sampled_idx)];
all_cidx = [cidx; cidx_val; aug_cidx(sampled_idx)];
rnd_idx = randperm(length(all_cidx));
all_fnames2 = all_fnames(rnd_idx);
all_cidx2 = all_cidx(rnd_idx);

fid = fopen(['data' filesep 'ucf101aug_K1_train_all.txt'], 'w');
