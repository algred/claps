...
%% idx = randsample(length(video_list), 5);
idx = [1 1000 2000 3000 4000 5000];
for i = 1:length(idx)
    vid = idx(i);
    ...
    for j = K : nfms
        ...
        hold on; 
        for bb = 1:size(new_bboxs, 1)
            rectangle('Position', new_bboxs(bb, :), ...
                'EdgeColor', 'y', 'LineWidth', 1.5); 
        end
%         for bb = 1:size(this_bboxs, 1)
%             rectangle('Position', this_bboxs(bb, 1:4), ...
